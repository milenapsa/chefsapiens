...
for batch in 0 1 2 3; do
  docker exec -e LEAD_ID="$ID" -e OFFSET="$((batch*110))" -e EVENTS=110 "$L" node --input-type=module -e "const id=process.env.LEAD_ID,o=+process.env.OFFSET,n=+process.env.EVENTS,k=process.env.ADMIN_API_KEY,b='http://127.0.0.1:8082';for(let j=1;j<=n;j++){const i=o+j,x={stage:i%2?'proposal':'contacted',owner:'Rotation Lab',nextAction:'Synthetic '+i,nextActionAt:new Date(Date.now()+i*60000).toISOString(),notes:'LAB_'+i};const r=await fetch(b+'/admin-api/pipeline/'+encodeURIComponent(id),{method:'PATCH',headers:{Origin:'https://rotation-lab.local','x-admin-key':k,'content-type':'application/json'},body:JSON.stringify(x)});if(!r.ok)throw Error('patch '+i+' '+r.status)}console.log('ROTATION_BATCH_OK offset='+o)"
  if [ "$batch" -lt 3 ]; then
    docker restart "$L">/dev/null
    sleep 3
  fi
done
docker exec "$L" node --input-type=module -e "const k=process.env.ADMIN_API_KEY,b='http://127.0.0.1:8082';const r=await fetch(b+'/admin-api/audit/status',{headers:{'x-admin-key':k}}),j=await r.json();if(!r.ok||j.data.archiveCount<1||j.data.manifestEntries<1||j.data.deletionEnabled!==false)throw Error(JSON.stringify(j));console.log(JSON.stringify(j.data))"
